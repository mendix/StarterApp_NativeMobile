...
import static mendixsso.implementation.utils.OpenIDUtils.*;
...
public class OpenIDHandler extends RequestHandler {
    ...
    public void processRequest(IMxRuntimeRequest req, IMxRuntimeResponse resp, String path) {
        ...
        try {
            if (LOGOFF.equalsIgnoreCase(path)) {
                ...
                ErrorPageHandler.serveErrorPage(req, resp, ErrorPageContent.REQUESTED_RESOURCE_PATH_NOT_FOUND);
            }
        } catch (Exception e) {
            ...
            if (req.getParameter(ERROR_DESCRIPTION) != null) {
                MendixSSOLogger.error(e, "An unexpected exception occurred while handling request : %s", req.getParameter(ERROR_DESCRIPTION));
                ErrorPageHandler.serveErrorPage(req, resp, ErrorPageContent.OOPS_UNEXPECTED_EXCEPTION_OCCURRED, req.getParameter(ERROR_DESCRIPTION));
            } else {
                MendixSSOLogger.error(e, "An unexpected exception occurred while handling request : %s", path);
                ErrorPageHandler.serveErrorPage(req, resp, ErrorPageContent.INTERNAL_SERVER_ERROR_UNEXPECTED_EXCEPTION_OCCURRED, path);
            }
        }
    }
    ...
    private void callback(IMxRuntimeRequest req, IMxRuntimeResponse resp, IContext context)
        ...
        try {
            ...
            if (nonce == null) {
                throw new IllegalStateException("Unexpected OpenID callback unknown nonce");
            }
            ...
            if (authResp instanceof AuthenticationErrorResponse) {
                ...
                final Nonce expectedHashedNonce = new Nonce(sha256Hex(nonce));
                ...
                final JWT idToken = getAndValidateIDToken(idTokenResponse, expectedHashedNonce);
    ...
    private void performAuthorizeRequest(
        ...
        final String hashedNonce = sha256Hex(nonce.getValue());
        ...
        final AuthenticationRequest.Builder authenticationRequestBuilder =
                new AuthenticationRequest.Builder(
                        ...
                        .state(state)
                        .nonce(new Nonce(hashedNonce))
                        .prompt(new Prompt(promptType));

        if (signupHint != null && !signupHint.isEmpty()) {
        ...
        setNonceCookie(req, resp, nonce.getValue());
